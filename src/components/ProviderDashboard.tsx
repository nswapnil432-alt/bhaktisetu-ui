...
export default function ProviderDashboard() {
  ...
  console.log("📊 REAL STATS FROM SERVER:", statsData); // 👈 Add this line!
  setStats({
    totalBookings: statsData.totalBookings || 0,
    totalEarnings: statsData.totalEarnings || 0,
    rating: statsData.rating || 0,
    totalReviews: statsData.totalReviews || 0,
    profileViews: statsData.profileViews || 0, // 🚩 This MUST be statsData.profileViews
    totalSevas: statsData.totalSevas || 0
  });
    } catch (error) {
      alert("Network error.");
              ...
              <div key={req.id} className="bg-white rounded-[1.5rem] p-5 shadow-sm border-2" style={{ borderColor: `${theme.color}30` }}>
                ...
                {req.status === 'CONFIRMED' && (
                  <div className="mt-4 flex gap-2">
                    <button
                      onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(req.eventLocation)}`, '_blank')} className="flex-1 py-3 rounded-xl bg-secondary text-black text-[13px] font-bold shadow-md flex items-center justify-center gap-2"
                    >
